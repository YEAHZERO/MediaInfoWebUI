...
func (r *screenshotRunner) acceptBitmapSubtitleCandidate(label string, candidate float64) (float64, bool) {
	...
	if err != nil {
		r.logf("[提示] %s候选可视性验证失败，沿用该时间点: %s | 原因: %v", label, screenshottimestamps.SecToHMSMS(candidate), err)
	...
	if !visible {
		if r != nil && r.subtitle.Mode == "internal" && r.isSupportedBitmapSubtitle() {
			...
			if longBack > shortBack {
				...
				if longErr == nil && longVisible {
					...
					r.logf("[提示] %s候选仅在较大回溯窗口下渲染出字幕，后续位图截图改用 %ds 回溯窗口: %s", label, longBack, screenshottimestamps.SecToHMSMS(candidate))
					return candidate, true
				}
			}
		}
		...
		r.logf("[提示] %s候选未实际渲染出字幕，继续搜索: %s", label, screenshottimestamps.SecToHMSMS(candidate))
		return 0, false
	}
	return candidate, true
}
