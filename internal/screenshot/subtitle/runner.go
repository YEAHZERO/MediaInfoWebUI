...
func (r *Runner) Choose() error {
	if r.SubtitleMode == subtitleModeOff {
		if r.Subtitle != nil {
			r.Subtitle.Mode = "none"
		}
		return nil
	}

	selection, ok, err := r.findExternalSubtitle()
	if err != nil {
		return err
	}
	if ok {
		*r.Subtitle = selection
		r.logf("[信息] 已选择外挂字幕：%s", selection.File)
		return nil
	}

	selection, ok, err = r.pickInternalSubtitle()
	if err != nil {
		return err
	}
	if ok {
